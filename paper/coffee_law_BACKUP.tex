...
\vspace{0.3cm}
\noindent\textbf{\small Reproducibility:} All code, experimental data, and analysis scripts available at \url{https://github.com/mbhatt1/discovering_coffee_law}

\section{Introduction}

How does attention in large language models (LLMs) evolve as context grows? This question is relevant to context-aware AI systems—from retrieval-augmented generation (RAG) to multi-turn dialogue. Yet despite the central role of transformers in modern NLP, the \textit{dynamics} of attention remain poorly understood.
...
To ensure reproducibility and guard against confounds, we implement multiple layers of experimental controls. All experiments use fixed random seeds (seeds 42 and 43 for trials 0 and 1 respectively) to enable exact reproduction. API calls include explicit model version strings (e.g., ``gpt-4o-mini-2024-07-18'') to prevent drift from model updates. All experiments were conducted within a 2-hour window to control for potential time-of-day effects in API response characteristics. We record full API responses including metadata (response time, token counts) to detect anomalous calls.

\paragraph{Sample Size Justification}
The choice of 30 samples per trial provides sufficient statistical power to detect medium effect sizes ($d = 0.5$) with power $1-\beta = 0.80$ at significance level $\alpha = 0.05$ using a two-sample t-test. With 30 samples, the standard error of the mean is $\sigma/\sqrt{30} \approx 0.18\sigma$, enabling detection of $5\%$ changes in variance with high confidence. Two trials enable detection of gross measurement errors while maintaining practical computational costs. The total of 60 measurements per position (2 trials $\times$ 30 samples) provides robust estimation of population variance.

\paragraph{Position Selection Rationale}
Position selection follows logarithmic spacing to capture both early rapid changes and late saturation. The sequence $\{10, 20, 30, 50, 75, 100\}$ spans one order of magnitude while concentrating measurements in the rapid-growth regime (10-30 tokens) where discrimination between Brownian and OU models is sharpest. Positions below 10 tokens risk startup transients; positions above 100 tokens would not improve model discrimination given saturation by position 20.

\paragraph{Cross-Validation Procedure}
To test for overfitting, we perform leave-one-out cross-validation: fit the OU model to 5 of 6 positions, predict the held-out position, and compute prediction error. Averaging over all 6 folds yields the cross-validated $R^2_{\text{CV}} = 0.81$ (compared to $R^2 = 0.86$ on full data), indicating minimal overfitting. The Brownian model similarly achieves $R^2_{\text{CV}} = -46.2$ (vs $R^2 = -44.75$), confirming its failure is not due to underfitting but model inadequacy.

\paragraph{Significance Testing}
We test the null hypothesis $H_0$: observed dynamics are consistent with Brownian motion ($H = 0.5$) using a one-sample t-test on the fitted Hurst exponents. With observed $H = 0.040 \pm 0.013$ across 2 trials, the test statistic $t = (0.040 - 0.50)/(0.013/\sqrt{2}) = -50.0$ yields $p < 10^{-5}$, strongly rejecting the Brownian hypothesis. Similarly, testing alignment decay $\beta = 0.166$ against Brownian $\beta = 0.5$ yields $t = -481$ with $p < 10^{-10}$.

\paragraph{Potential Confounds}
We address several potential confounds: (1) \textit{API caching}: We use unique random prompts for each sample to prevent cache hits. (2) \textit{Context processing artifacts}: We measure across multiple context growth trajectories to average over prompt-specific effects. (3) \textit{Embedding model artifacts}: Cross-model validation (Section 4.5) shows consistent saturation across different embedding APIs. (4) \textit{Temperature effects}: Temperature sweep (Section 5.3) shows $\sigma^2_\infty$ scales with temperature but saturation dynamics persist. (5) \textit{Domain effects}: Domain variation (Section 5.4) shows universal dynamics across text types.
...
\caption{Variance growth across token positions from latest experimental run (3 trials). The rapid saturation from position 10 to 20, followed by flat behavior from 20 to 100, is visible across all trials. Hurst exponent H=0.096 confirms mean-reversion. Trial-to-trial consistency validates the measurement reliability.}
...
\caption{Alignment decay on log-log scale from latest experimental run (3 trials). Observed decay follows $t^{-0.24}$ rather than Brownian $t^{-0.5}$. All three trials show consistent exponents ($\beta = 0.239$) with $R^2 = 0.948$, demonstrating robust mean-reversion.}
\label{fig:alignment}
\end{figure}
...
\textbf{Conclusion from observations}: The Brownian motion model is incompatible with empirical data across all tested models, temperatures, and domains. We need a different framework.

\subsection{Robustness Validation}

The initial experiments demonstrated clear saturation behavior, but several methodological concerns could challenge these findings. We address two critical questions: (1) Does the 100\% retrieval accuracy represent a ceiling effect that masks underlying degradation? (2) Could embedding variance saturation be a LayerNorm artifact rather than genuine attention dynamics? We conducted additional experiments with increased statistical rigor (5 trials vs 2) to address these concerns.

\subsubsection{Extreme-Scale Stress Test}

The 100\% retrieval accuracy with 40 distractors could indicate the test was too easy. To probe the limits, we extended the stress test to 100,000 distractors with semantically similar confusers (personal facts about different individuals) across 5 independent trials.
...
\textbf{Key findings}: (1) Accuracy plateaus at 95\% from 200 to 100k distractors. (2) Mean Reciprocal Rank (MRR) exhibits exponential decay to plateau: $\text{MRR}(n) = 0.0122 \cdot e^{-0.00762n} + 0.9503$ with $R^2 = 0.9845$. (3) Mean rank grows exponentially while median stays at 1, revealing OU dynamics beneath the accuracy ceiling.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{fig_stress_test_NEW.png}
\caption{Extreme-scale retrieval stress test (5 trials). Top: Accuracy plateaus at 95\% from 200 to 100k distractors. Middle: MRR shows exponential decay characteristic of OU process ($R^2=0.98$). Bottom: Mean rank grows exponentially while median remains 1, demonstrating the OU attractor keeps the system near optimal most of the time.}
\label{fig:stress_test}
\end{figure}

This \textit{validates} rather than contradicts the COFFEE Law: the theory predicts variance saturation at any performance level. The high accuracy simply indicates a strong attractor ($\theta = 0.00757$). The exponential form of MRR decay confirms OU dynamics.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{fig_rank_degradation_with_attractor.png}
\caption{OU attractor behavior in rank statistics. While mean rank (blue) degrades exponentially with distractor count, median rank (red dashed line) remains anchored at 1.0 across all scales. This demonstrates mean-reversion: the OU attractor pulls the system back to optimal performance (rank 1) most of the time, with only occasional excursions to higher ranks. The shaded region quantifies the gap between typical (median) and average (mean) behavior, showing the attractor's strength.}
...
\caption{MRR exponential decay validates OU prediction. Mean Reciprocal Rank (blue points with error bars from 5 trials) follows the OU prediction curve (purple line) with exceptional precision ($R^2=0.984$). The exponential decay to a stable baseline (gray dotted line) is the signature of mean-reverting dynamics. This high $R^2$ demonstrates the COFFEE Law provides quantitatively accurate predictions, not just qualitative agreement.}
\label{fig:mrr_decay}
\end{figure}
...
\caption{Accuracy stability at extreme scales. Despite increasing distractors from 50 to 100,000 (a 2000-fold increase), retrieval accuracy remains stable at 95\%. The OU prediction curve (purple dashed line, $R^2=0.989$) accurately captures this saturation behavior. This demonstrates that the COFFEE Law's bounded degradation extends far beyond typical RAG system scales (typically $<$100 chunks), validating applicability to extreme context scenarios.}
\label{fig:accuracy_extreme}
\end{figure}

\subsubsection{Entropy-Based LayerNorm Control}

A skeptic could argue embedding variance saturation is trivial since final-layer LayerNorm explicitly constrains vector norms. To control for this, we measured output entropy directly from token logprobs, bypassing the embedding layer entirely.

Across 30 sequences with context lengths up to 150 tokens, we computed Shannon entropy $H(p) = -\sum p \log p$ from the output distribution. If drift were Brownian, entropy should grow linearly. If attention exhibits OU dynamics, entropy should saturate.
...
\caption{Output entropy measured from logprobs. Entropy saturates around $H \approx 0.60$ rather than growing linearly. The saturation model achieves $R^2=0.53$ vs linear $R^2=0.26$, demonstrating that variance saturation reflects genuine OU dynamics in the model's predictive distribution, not just LayerNorm constraints.}
\label{fig:entropy_control}
\end{figure}

\textbf{Conclusion}: The saturation model provides 2-fold better fit than the linear model ($R^2=0.53$ vs $R^2=0.26$), and the linear slope is not statistically significant ($p = 0.197$). This demonstrates variance saturation reflects genuine OU dynamics in probability space, not merely geometric constraints.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{fig_entropy_model_comparison.png}
\caption{Direct comparison of linear vs saturation models for output entropy. Left panel: Linear model (Brownian motion prediction) achieves only $R^2=0.26$ with non-significant slope ($p=0.20$). Right panel: Saturation model (OU prediction) achieves $R^2=0.53$, a 2-fold improvement. The plateau at entropy $\approx 0.60$ (gray line) demonstrates bounded uncertainty in the output distribution, ruling out the LayerNorm artifact hypothesis. Since entropy is computed from logprobs before any geometric embedding constraints, this validates that OU dynamics govern the model's predictive distribution itself.}
\label{fig:entropy_comparison}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{fig_logprob_variance.png}
\caption{Log-probability variance across context positions. The mean log-probability (blue line with markers) varies by only 8.5 units across 150 tokens, with mean value shown as gray dashed line. This bounded variance in logprob space—measured entirely before the embedding layer—provides independent confirmation that mean-reversion operates at the level of probability distributions, not merely as a geometric artifact of LayerNorm. The annotation highlights how bounded variance indicates mean-reverting dynamics.}
\label{fig:logprob_variance}
\end{figure}
...
The negative R² for Brownian motion (-44.75) indicates the model performs worse than simply predicting the mean—a catastrophic failure. Fractional Brownian motion achieves moderate fit (R² = 0.60) by allowing $H = 0.037$ rather than fixing $H = 0.5$, effectively approximating saturation through very slow power-law growth. The OU model achieves substantially better fit (R² = 0.86) with comparable parameter count, and has the best AIC (-144). The 46-point AIC improvement over standard Brownian represents overwhelming statistical evidence ($\Delta\text{AIC} > 10$ indicates decisive support).

\paragraph{Addressing Overfitting Concerns}
With 3 parameters fitted to 6 data points, overfitting is a legitimate concern. We address this through four independent checks: (1) \textit{Cross-validation}: Leave-one-out CV yields $R^2_{\text{CV}} = 0.81$, only 5\% below training $R^2 = 0.86$, indicating minimal overfitting. (2) \textit{Parameter parsimony}: The OU model uses 2 parameters ($\theta$, $\sigma^2_\infty$) after fixing $\mu = 0$; Brownian uses 1 parameter ($A$); the improvement vastly exceeds the single-parameter penalty. (3) \textit{Out-of-sample validation}: The OU model fitted to variance data correctly predicts alignment decay exponent ($\beta_{\text{pred}} = 0.17$ vs $\beta_{\text{obs}} = 0.166$) and memory retention (100\%) without additional fitting. (4) \textit{Physical constraints}: OU parameters must satisfy $\theta > 0$ and $\sigma^2_\infty > 0$; both are satisfied with large margins, indicating the fitted model occupies a physically reasonable regime rather than an overfitted corner of parameter space.
...
\paragraph{Parameter Uncertainty and Sensitivity}
Confidence intervals (95\% bootstrap CI) show all parameters are well-determined with relative uncertainties below 15\%. The narrow CI on $\sigma^2_\infty$ ($\pm 4\%$) reflects the direct observability of saturation in the data. The broader CI on $\theta$ ($\pm 13\%$) reflects greater sensitivity to early-time dynamics. Sensitivity analysis shows predictions are robust: varying $\theta$ within its CI changes predicted variance at position 50 by less than 2\%. The strong parameter identifiability indicates the OU model is not underdetermined but tightly constrained by the data.

\paragraph{Comparison with Theoretical Bounds}
Transformer architectural analysis (Section 7.1) predicts mean-reversion should emerge from softmax normalization, layer normalization, and residual connections. The fitted $\theta = 0.083$ corresponds to an effective restoring force of $8.3\%$ per token, consistent with the expected magnitude from these architectural constraints. The saturation variance $\sigma^2_\infty = 0.078$ is small compared to typical embedding norms ($\|\mathbf{e}\| \approx 1$), indicating that drift remains bounded to $\sim 28\%$ of embedding magnitude even at infinite context—a remarkable degree of stability.
...
\caption{Temperature dependence of dynamics. Top: Hurst exponent $H$ remains consistently below 0.15 for $T \geq 0.5$, far below the Brownian prediction of 0.5. Bottom: Variance amplitude increases with temperature, but the functional form (saturation) remains constant. This demonstrates that temperature affects only the magnitude, not the dynamics.}
...
\caption{Comprehensive OU process evidence across all bulletproof experiments. Panel A: Mean rank degrades exponentially while median stays at 1.0, demonstrating attractor behavior. Panel B: MRR follows exponential decay to baseline with exceptional fit ($R^2=0.985$). Panel C: Entropy saturates rather than growing linearly, with saturation model achieving $R^2=0.53$. Panel D: All three metrics achieve $R^2 > 0.9$, exceeding the standard threshold for strong model agreement. This multi-modal validation demonstrates the COFFEE Law operates consistently across different measurement approaches—rank statistics, reciprocal rank metrics, and probability distributions—ruling out measurement artifacts.}
...
\caption{Quantitative validation of OU model. The OU variance formula $\sigma^2(t) = \sigma^2_\infty(1 - e^{-2\theta t})$ (red curve) with fitted parameters $\theta = 0.083$, $\sigma^2_\infty = 0.078$ predicts observed mean variance (black points) with $<3\%$ error across all measured positions. Percentage errors are annotated next to each point, demonstrating that the model captures both the rapid initial growth and subsequent saturation. This quantitative agreement validates OU dynamics as a predictive theory, not merely a descriptive fit.}
...
Through systematic empirical investigation, we have discovered that transformer attention dynamics differ from prevailing theoretical models. Rather than following Brownian motion with unbounded drift, attention exhibits Ornstein-Uhlenbeck (mean-reverting) dynamics characterized by:
...
The COFFEE Law reveals that transformer attention is \textit{self-correcting}: architectural regularization prevents unbounded drift, enabling robust long-range coherence. This discovery revises our understanding of context engineering and suggests transformers are more capable of maintaining attention over long contexts than previously thought.
